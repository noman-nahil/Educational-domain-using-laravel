...
<body data-spy="scroll" data-target=".site-navbar-target" data-offset="300">
  ...
  <div class="site-wrap">
    ...
    <table class="table">
      ...
      @for($i=0; $i<count($noticePost); $i++)
			<tr>
				<td>{{$noticePost[$i]['teacherid']}}</td>
				<td>{{$noticePost[$i]['notice']}}</td>
				<td>{{$noticePost[$i]['dateTime']}}</td>
				<td>
					<a href="/teacher/edit/{{$noticePost[$i]['id']}}">Edit</a>
        </td>
        <td>
          <a href="/teacher/delete/{{$noticePost[$i]['id']}}">Delete</a>
				</td>
			</tr>
            @endfor
      </tbody>
